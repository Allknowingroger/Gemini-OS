  ...
  useEffect(() => {
    ...
    if (!isLoading) {
      if (htmlContent !== processedHtmlContentRef.current) {
        // Fix: Explicitly type the scripts collection to avoid 'unknown' type errors during iteration
        const scripts = Array.from(container.getElementsByTagName('script')) as HTMLScriptElement[];
        scripts.forEach((oldScript) => {
          try {
            const newScript = document.createElement('script');
            // Fix: Iterating over attributes of typed HTMLScriptElement and casting attr to any to avoid property access errors
            ...
            console.error(
              ...
              {
                // Fix: Access innerHTML safely on the typed script element
                scriptContent:
